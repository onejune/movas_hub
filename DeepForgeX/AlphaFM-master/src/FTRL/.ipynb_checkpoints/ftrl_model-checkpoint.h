...
bool ftrl_model<T>::output_txt_model(const string& modelPath)
{
    ...
    if(!out) return false;
    out << biasFeaName << "\002" << muBias->wi << "\002" << muBias->w_ni << "\002" << muBias->w_zi << endl;
    for(auto iter = muMap.begin(); iter != muMap.end(); ++iter)
    {
        out << iter->first << "\002" << iter->second << endl;
    }
    out.close();
    return true;
}
...
void ftrl_model<T>::output_model_one_line(ostream& out, const char* feaName, ftrl_model_unit<T>* pMu, bool isBias)
{
    if(isBias)
    {
        out << feaName << "\002" << pMu->wi << "\002" << pMu->w_ni << "\002" << pMu->w_zi << endl;
    }
    else
    {
        out << feaName << "\002" << *pMu << endl;
    }
}
...
bool ftrl_model<T>::load_txt_model(const string& modelPath)
{
    ...
    if(!in) {
        cout << "load txt model error for ifstream." << endl;
        return false;
    }    
...
bool ftrl_model<T>::load_txt_model(ifstream& in)
{
    ...
    utils::split_string(line, '\002', &strVec);
    if(strVec.size() != 4)
    {
        cout << "invalid para count:" << strVec.size() << endl;
        return false;
    }
    ...
    while(getline(in, line))
    {
        ...
        {
            cout << "invalid data: factor_num=" << factor_num << " para count=" << strVec.size() << endl; 
            return false;
        }
