...
{
    this->load_feature_config();
    pModel = new predict_model<T>(opt.factor_num);
    cout << "load model...\n" << "model_path=" << opt.model_path << ", model_format=" << opt.model_format << ", factor_num=" << opt.factor_num << endl;
...
void ftrl_predictor<T>::run_task(vector<string>& dataBuffer)
{
    ...
    {
        fm_sample sample(dataBuffer[i], this->column_names, this->selected_features, this->label_index);
        //fm_sample sample(dataBuffer[i], this->column_names, this->combine_fea_list, this->label_index);
