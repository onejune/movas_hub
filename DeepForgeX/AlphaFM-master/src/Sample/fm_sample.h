...
public:
    ...
    fm_sample(const string& line, const vector<string>& column_names, const unordered_map<string, bool>& selected_features, int label_index);
    fm_sample(const string& line, 
...
const unordered_set<string> fm_sample::invalid_values = {"-1.0", "-", ""};
...
fm_sample::fm_sample(const string& line, const vector<string>& column_names, 
                     const vector<vector<string>>& combine_schema_feature, 
                     int label_index)
{
    this->x.clear();
...
// 辅助函数：生成所有可能的组合特征
void fm_sample::generate_combinations(const vector<string>& components, 
                                      const vector<vector<string>>& component_value_lists,
                                      size_t index, string current_key, 
                                      vector<string>& combined_keys)
{
