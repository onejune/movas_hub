...
class MsModelTrainFlow:
    ...
    def _read_dataset_by_date(self, base_path, date_str):
        data_path = os.path.join(base_path, f"sampled_{date_str}")
        ...
        df = df.filter(F.col("dp").isin([0, 1]))
    ...
    def _train_model(self, train_dataset, model_in_path_current, model_out_path_current, model_version_current):
        ...
        estimator = ms.PyTorchEstimator(module=self.model_module,
                                      ...
                                      input_label_column_name='dp',
                                      output_label_column_name='dp',
    ...
    def _evaluate_model(self, test_result_df, test_date_str_formatted):
        ...
        for pkg_name in pkg_names:
            ...
            label_pred_list = (
                filtered_df.select("dp", "rawPrediction")
                .rdd.map(lambda row: (float(row.dp), float(row.rawPrediction)))
                .collect()
            )
            # lebel_pred_list = [(x[0], x[1]) for x in label_pred_list]
            label_pred_list = [(x[0], 0.1*x[1]/(1-0.9*x[1])) for x in label_pred_list]            
            # 初始化评估指标和样本数
            auc, pcoc = 0.0, 0.0
        ...
        # 使用 tabulate 生成表格字符串
        headers = ["DATE", "Key", "AUC", "PCOC", "D-PURCHASE", "IMPRESSION", "IVR"]
    ...
    def _run_training_loop(self):
        ...
        while current_date <= self.train_end_date_dt:
            ...
            # 检查数据是否存在
            data_path = os.path.join(self.train_path_prefix, f"sampled_{date_str}")
            if not os.path.exists(data_path):
                MovasLogger.add_log(f"Data not found for date {date_str}: {data_path}. Skipping training for this date.")
            ...
            # 更新 current_model_in_path 为本轮输出模型路径
