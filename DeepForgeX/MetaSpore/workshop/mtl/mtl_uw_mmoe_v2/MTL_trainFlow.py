...
from metaspore.algos.pepnet2_net import PEPNet2

from metaspore.algos.multitask import MtlMMoEModel, MdlMMoEModel, MMoE, MtlSharedBottomModel, SceneAwareMMoE, HoME, UncerWgtMMoE
...
class MTLModelTrainFlow(MsModelTrainFlow):
    ...
    def _build_model_module(self):
        ...
        elif configed_model == "uw_mmoe": # MMoE with Uncertainty Weighting
            self.model_module = UncerWgtMMoE(
                embedding_dim=self.embedding_size,
                combine_schema_path=self.combine_schema_path,
                expert_numb=self.expert_numb,
                task_numb=self.tasks_cnt,
                expert_out_dim=8,
                expert_hidden_units=self.dnn_hidden_units,
                tower_hidden_units=[32],
                ...
                ftrl_alpha=self.ftrl_alpha,
                ftrl_beta=self.ftrl_beta,
                use_uncertainty_weighting=True,  # 启用不确定性权重
                initial_log_var=[0.0] * self.tasks_cnt,      # 初始对数方差
            )
        elif configed_model == "sa_mmoe":
