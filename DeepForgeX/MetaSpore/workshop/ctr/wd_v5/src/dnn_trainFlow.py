...
from metaspore.algos.winrate.deep_censored_model import DeepCensoredModel
...
class MsModelTrainFlow:
    ...
    def _read_dataset_by_date(self, base_path, date_str):
        ...
        df = df.fillna('') #important
        #df = df.repartition(200)  # 减少分区数，让每个分区更大
        MovasLogger.log(f'after random_sample: sample_count={df.count()}')

        return df

    def _read_multiple_dates_dataset(self, base_path, start_date, end_date):
        """读取多天的数据集"""
        current_date = start_date

        parquet_paths_to_read = []
        while current_date <= end_date:
            date_str = current_date.strftime("%Y-%m-%d")
            daily_path = os.path.join(base_path, date_str)
            if not os.path.exists(daily_path):
                daily_path = os.path.join(base_path, f"part={date_str}")
            if os.path.exists(daily_path):
                parquet_paths_to_read.append(daily_path)
            current_date += timedelta(days=1)

        if not parquet_paths_to_read:
            raise ValueError(f"No valid paths found for dates between {start_date.strftime('%Y-%m-%d')} and {end_date.strftime('%Y-%m-%d')}")
        MovasLogger.log(f"Attempting to read data from paths: {parquet_paths_to_read}")
            
        combined_df = self.spark_session.read.parquet(*parquet_paths_to_read)
        # 负采样、预处理
        df = self._preprocess_dataset(combined_df)
        return df
    ...
    def write_validation_results_to_csv(self, sorted_results, test_date_str_formatted):
        ...
        exp_info = f"readme: {readme}; batchsize: {self.batch_size}; emb_size: {self.embedding_size}; sparse_optimizer_type: {self.sparse_optimizer_type};"
        # 添加额外的列
    ...
    def _run_training_loop(self):
        ...
        while current_date <= self.train_end_date_dt:
            ...
            # 计算当前批次的结束日期
            batch_end_date = min(current_date + timedelta(days=self.batch_days-1), self.train_end_date_dt)
            batch_start_date_str = current_date.strftime("%Y-%m-%d")
            batch_end_date_str = batch_end_date.strftime("%Y-%m-%d")
            
            MovasLogger.log(f"--- Training for date range: {batch_start_date_str} to {batch_end_date_str} ({self.batch_days} days) ---")

            if not os.path.exists(self.model_out_base_path):
                os.makedirs(self.model_out_base_path, exist_ok=True)
                
            # 模型输出路径使用批次的结束日期
            model_out_path_current = os.path.join(self.model_out_base_path, f"model_{batch_end_date_str}")
            
            # 读取多天的数据进行训练
            train_dataset = self._read_multiple_dates_dataset(
                self.train_path_prefix, 
                current_date, 
                batch_end_date
            )
            self._train_model(train_dataset, current_model_in_path, model_out_path_current, batch_end_date_str)

            current_model_in_path = model_out_path_current
            self.delete_directories_before_date(self.model_out_base_path, batch_end_date_str)
    ...
    def _run_evaluation_phase(self):
        if self.trained_model_path and self.validation_date: 
            ...
            sample_date_dt = datetime.strptime(test_date_str_formatted, '%Y-%m-%d')
            train_dataset = self._read_multiple_dates_dataset(
                self.train_path_prefix, 
                sample_date_dt, 
                sample_date_dt
            ...
            test_result_df = self._predict_data(train_dataset, self.trained_model_path)
