...
def set_updater(embedding_layer, optimizer_type, **kwargs):
    """
    Set the updater for an embedding layer based on the specified optimizer type.
    
    Args:
        embedding_layer: The metaspore EmbeddingSumConcat layer or any layer with an 'updater' attribute
        optimizer_type: String specifying the optimizer ('ftrl', 'adam', 'sgd', 'adagrad', 'adamw', 'noop')
        **kwargs: Additional parameters for specific optimizers
    """
    valid_optimizers = ['ftrl', 'adam', 'sgd', 'adagrad', 'adamw', 'noop']
    ...
    if optimizer_type == 'ftrl':
        # Extract FTRL-specific parameters from kwargs or use defaults
        l1 = kwargs.get('l1', 1.0)
        l2 = kwargs.get('l2', 120.0)
        alpha = kwargs.get('alpha', 0.5)
        beta = kwargs.get('beta', 1.0)
        embedding_layer.updater = FTRLTensorUpdater(l1=l1, l2=l2, alpha=alpha, beta=beta)
    elif optimizer_type == 'adam':
        # Extract Adam-specific parameters from kwargs or use defaults
        learning_rate = kwargs.get('learning_rate', 1e-6)
        beta1 = kwargs.get('beta1', 0.9)
        beta2 = kwargs.get('beta2', 0.999)
        epsilon = kwargs.get('epsilon', 1e-8)
        embedding_layer.updater = AdamTensorUpdater(learning_rate=learning_rate, 
                                                      beta1=beta1, 
                                                      beta2=beta2, 
                                                      epsilon=epsilon)
    elif optimizer_type == 'sgd':
        # Extract SGD-specific parameters from kwargs or use defaults
        learning_rate = kwargs.get('learning_rate', 1e-6)
        embedding_layer.updater = SGDTensorUpdater(learning_rate=learning_rate)
    elif optimizer_type == 'adagrad':
        # Extract AdaGrad-specific parameters from kwargs or use defaults
        learning_rate = kwargs.get('learning_rate', 1e-6)
        float_stable_eps = kwargs.get('float_stable_eps', 0.0)
        l2_reg = kwargs.get('l2', 0.0)
        embedding_layer.updater = AdaGradTensorUpdater(learning_rate=learning_rate, 
                                                         float_stable_eps=float_stable_eps, 
                                                         l2=l2_reg)
    elif optimizer_type == 'adamw':
        # Extract AdamW-specific parameters from kwargs or use defaults
        learning_rate = kwargs.get('learning_rate', 1e-6)
        beta1 = kwargs.get('beta1', 0.9)
        beta2 = kwargs.get('beta2', 0.999)
        epsilon = kwargs.get('epsilon', 1e-8)
        ...
        embedding_layer.updater = AdamWTensorUpdater(learning_rate=learning_rate, 
                                                       beta1=beta1, 
                                                       beta2=beta2, 
                                                       epsilon=epsilon,
                                                       weight_decay=weight_decay,
                                                       amsgrad=amsgrad)
    elif optimizer_type == 'noop':
        # No operation updater
        embedding_layer.updater = NoOpUpdater(learning_rate=kwargs.get('learning_rate', 0.0))
