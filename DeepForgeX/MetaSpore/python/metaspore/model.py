...
class SparseModel(Model):
    ...
    def __call__(self, minibatch):
        ...
        ############# 这里调用自定义的Model 类的 do_extra_work() 方法，做一些额外的处理 ########
        if hasattr(self._module, 'do_extra_work') and callable(getattr(self._module, 'do_extra_work')):
            self._module.do_extra_work(minibatch)
        ##################################################################################
