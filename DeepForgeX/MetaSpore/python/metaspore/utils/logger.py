...
class LogUnit:
    ...
    def format(self):
        if self.caller:
            ...
            s = '[%s][%s][%s:%s:%s] %s' % (
                self.log_time, self.level, filename, 
                self.caller.lineno, self.caller.function, self.content
            )
        else:
            s = '[%s][%s] %s' % (self.log_time, self.level, self.content)
        return s
...
class MovasLogger:
    ...
    使用方式:
        MovasLogger.init(spark, output_path)
        MovasLogger.log("消息内容")
