...
# -*- coding: utf-8 -*-
"""
metaspore/trainflows/base.py - MetaSpore 训练流程基类

提取 dnn_trainFlow.py 中的公共模块，供所有 trainFlow 子类继承。

子类需要实现:
- _build_model_module(): 构建具体模型

子类可选重写:
- _read_dataset_by_date(): 自定义数据读取逻辑
- _preprocess() / _postprocess(): 训练前后的钩子
"""
import os
import sys
import io
...
from metaspore.utils import FeishuNotifier
...
class BaseTrainFlow:
    ...
    公共模块:
    - Config 加载: _load_config, _load_combine_schema, _initialize_params
    - Spark 管理: _init_spark, _stop_spark
    - 数据采样: random_sample, random_sample2, random_sample3
    - 数据读取: _read_dataset_by_date (可被子类重写)
    - 训练循环: _run_training_loop, delete_directories_before_date
    - 模型训练: _train_model
    - 预测评估: _predict_data, _evaluate_model, _eval_by_df, write_validation_results_to_csv
    - 主流程: run_complete_flow, parse_args
    - 钩子: _preprocess, _postprocess
    ...
    def __init__(self, config_path: str):
        ...
        self.gradnorm_alpha = 0.0001
        self.use_uncertainty_weighting = False
        self.sparse_optimizer_type = 'adam'
    ...
    def _load_config(self, path: str) -> dict:
        ...
        with open(path, 'r') as stream:
            ...
            print(f'Debug--load config: {params}')
        required_keys = [
            'train_path_prefix', 'model_out_base_path', 'worker_count',
            ...
            'train_start_date', 'train_end_date', 'validation_date', 'movas_log_output_path'
        ]
    ...
    def _initialize_params(self):
        ...
        if not hasattr(self, 'movas_log_output_path'):
            self.movas_log_output_path = self.params.get('movas_log_output_path', 'base_ms_movas_log.txt')
    ...
    def _init_spark(self):
        MovasLogger.add_log(content='Debug -- spark init')
        spark_confs = {
        ...
        try:
            ...
            MovasLogger.add_log(level='ERROR', content="'zip' command not found or '../MetaSpore/python' does not exist.")

        self.spark_session = ms.spark.get_session(
            local=self.local_spark,
    ...
    def random_sample3(self, df):
        return df.filter(F.rand(seed=42) < 0.001)
    ...
    def _build_model_module(self):
        raise NotImplementedError("Subclasses must implement _build_model_module()")
    ...
    def _train_model(self, train_dataset, model_in_path_current, model_out_path_current, model_version_current):
        ...
        MovasLogger.log(
            f"Starting training for version: {model_version_current}, "
            f"Model In: {model_in_path_current}, Model Out: {model_out_path_current}, "
            f"model_type: {self.model_type}, agent_class: {self.agent_class}, "
            f"configed_loss_func: {self.loss_func}, loss_func: {loss_func}"
        )
    ...
    def _predict_data(self, dataset_to_transform, model_in_path_current):
        ...
        return model_transformer.transform(dataset_to_transform)
    ...
    def _evaluate_model(self, test_result_df, test_date_str_formatted):
        ...
        for col_name in eval_col_list:
            if col_name == "Overall":
                self._eval_by_df("Overall", col_name, test_result_df, results)
            elif col_name not in test_result_df.columns:
                MovasLogger.log(f"错误: 列 '{col_name}' 不存在于DataFrame中，跳过")
                continue
            else:
        ...
        filtered_results = {k: v for k, v in results.items() if v.pos >= 50}
        ...
        table_data = [
            [tag, r.key1, r.key2, round(r.auc, 4), round(r.pcoc, 4), round(r.loss, 4), r.pos, r.neg, round(r.ivr, 4)]
        ...
        msg_text = (
            f"train_start_time: {self.train_start_formatted_dtm}"
        ...
        table_title = f"Validation Result For [{test_date_str_formatted}]"
        msg_title = f"{self.name} [{test_date_str_formatted}]"
    ...
    def write_validation_results_to_csv(self, sorted_results, test_date_str_formatted):
        table_data = [
            [self.name, r.key1, r.key2, round(r.auc, 4), round(r.pcoc, 4),
             round(r.loss, 4), r.pos, r.neg, round(r.ivr, 4)]
            for r in sorted_results
        ]
        headers = ["Model", "Key1", "Key2", "AUC", "PCOC", "LogLoss", "Pos", "Neg", "Ivr"]
        df = pd.DataFrame(table_data, columns=headers)
        readme = self.exp_readme.replace('\n', ' ').rstrip(';')
        exp_info = (
            f"readme: {readme}; batchsize: {self.batch_size}; "
            f"emb_size: {self.embedding_size}; sparse_optimizer_type: {self.sparse_optimizer_type};"
        )
    ...
    def _eval_by_df(self, col_name, col_value, filtered_df, result_dict):
        from metaspore.utils import compute_auc_pcoc, calculate_logloss
        ...
        result_dict[col_value] = EvalResult(
            key1=col_name, key2=col_value,
            auc=auc, pcoc=pcoc, loss=logloss,
            pos=positive_count, neg=impression, ivr=ivr
        )
    ...
    def _run_training_loop(self):
        ...
        if os.path.exists(previous_model_path):
            ...
            MovasLogger.log(f"No model found for previous day. Starting from empty model.")
        ...
        while current_date <= self.train_end_date_dt:
            if not os.path.exists("train_interrupt.flag"):
                MovasLogger.log("train_interrupt.flag not found. Stopping training.")
                break
    ...
    def _run_evaluation_phase(self):
        if self.trained_model_path and self.validation_date:
            ...
            MovasLogger.log('=' * 50 + f" Evaluating: {self.trained_model_path}, val_date: {test_date_str_formatted} " + '=' * 50)
    ...
    @how_much_time
    def _run_evaluation_manual(self, model_date: str, sample_date: str):
        self.trained_model_path = os.path.join(self.model_out_base_path, f"model_{model_date}")
        self.validation_date = sample_date
        if self.trained_model_path and self.validation_date:
            MovasLogger.log('=' * 20 + f" Evaluating: {self.trained_model_path}, eval date: {sample_date} " + '=' * 20)
            test_dataset = self._read_dataset_by_date(self.train_path_prefix, sample_date)
            test_result_df = self._predict_data(test_dataset, self.trained_model_path)
            self._evaluate_model(test_result_df, sample_date)
        else:
            MovasLogger.add_log(level='WARNING', content="No model was trained, skipping evaluation.")

    def delete_directories_before_date(self, directory: str, specified_date_str: str, days: int = 7):
        try:
    ...
    def parse_args():
        ...
        parser.add_argument('--validation', type=bool, required=False, help='validation only mode')
    ...
    def main(cls):
        ...
        """
        args = cls.parse_args()
        flow = cls(config_path=args.conf)
        flow.run_complete_flow(args)
