...
train_data="../../click_v1/sample_offline/sample_$sample_date"

if [[ ! -f "$train_data" ]]; then
    echo "$train_data 不存在,训练终止!"
fi 
cat ${train_data} | awk -F'\002' '{if($13=="COM.ZZKKO") print $0}' | ../model_bin/plm_predict $para -m ${model_path} -out ${model_path}.predict -core 8 -piece_num 8
