...
def _evaluation(node, output):
    ...
    if sum(num_clicks)<10:
        if sum(num_clicks)>0:
            output.write("-" * 50 + "\n")
            output.write("%s|NoConfidenceData\n" % (node.path))
...
def main(input_file, output_file):
    ...
    #_walk_tree(root, _print_path, sys.stderr)
    _walk_tree(root, _evaluation, output)
    output.close()

#    return auc, nwmae, wrmse, sum(num_clicks),\
#           sum(num_impressions) - sum(num_clicks)
