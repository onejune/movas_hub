...
struct trainer_option
{
    ...
    void parse_option(const vector<string>& args) //args转实际参数值，匹配i对应参数，i+1为对应参数值
    {
        int argc = args.size();
...
void ftrl_trainer::run_task(vector<string>& dataBuffer)
{
    for(int i = 0; i < dataBuffer.size(); ++i)
    {
        ...
        //plm_sample sample(dataBuffer[i], this->column_names, this->selected_features, this->label_index);
        plm_sample sample(dataBuffer[i], this->column_names, this->combine_fea_list, this->label_index);
