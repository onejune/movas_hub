...
# 配置区域
OSS_SAMPLE_PATH="/mnt/data/oss_dsp_algo/ivr/sample/ivr_sample_v7/csv"
#OSS_SAMPLE_PATH="/mnt/data/oss_dsp_algo/ivr/sample/ivr_sample_v7/csv"
OSS_ROOT=""
...
ftrl_jar_path="/mnt/workspace/walter.wan/git_project/movas_hub/DeepForgeX/FTRL-Proximal-LR/target/ftrl_maven_walter-0.0.1-SNAPSHOT.jar"
ftrl_jar_path="/mnt/workspace/walter.wan/git_project/movas_hub/DeepForgeX/FTRL-Proximal-LR/target/ftrl_maven_walter-0.0.1-SNAPSHOT.jar"
...
function init_env() {
    ...
    cp -n $SCRIPT_DIR/score_kdd.py $current_dir/
    cp -n $SCRIPT_DIR/figure_auc_regression.py $current_dir/
    cp -n $SCRIPT_DIR/feishu_notifier.py $current_dir/
}
...
function prepare_training_data_from_mnt(){
    ...
    if [[ "$OSS_SAMPLE_PATH" == *"ruf_sample_v1"* ]]; then
        local_data_path=$(find "$OSS_SAMPLE_PATH/$current_date" -maxdepth 1 -type f -name "*.csv" | head -n 1)
    else
...
#使用线上的模型进行验证
function validation_online_model_OLD(){
    local model_dir=$1
    local model_date=$2
    local sample_date=$3
...
function validation_online_OLD_by_filted_sample(){
    local model_dir=$1
    local model_date=$2
    local sample_date=$3
    ...
    #--------------------------- 过滤样本 ------------------------------
    awk -F'\002' '{if($11=="US") print $0}' $local_data_path > ./temp/sample_${sample_date}.dat
    local_data_path=./temp/sample_${sample_date}.dat
    #--------------------------- 过滤样本 ------------------------------
...
function train_by_hour() {
    ...
    while [[ "$current_timestamp" -le "$end_timestamp" ]]; do
        ...
        local_data_path=$(find "$OSS_SAMPLE_PATH/${current_date}/${current_datetime_str}/" -maxdepth 1 -type f -name "*.csv" | head -n 1)
