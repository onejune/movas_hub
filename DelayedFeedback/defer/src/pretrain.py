...
def test(model, test_data, params, before=False):
    ...
    if params["method"] == "3class":
        ...
        #all_logits = np.reshape(np.concatenate(all_logits, axis=0), (-1, 2))
        # delay_win_adapt 使用 winadapt 模型，有 4 个输出
...
def run(params):
    ...
    for ep in range(params["epoch"]):
        ...
        if params.get("model_ckpt_path"):
            import os; os.makedirs(params["model_ckpt_path"], exist_ok=True)
            model.save_weights(params["model_ckpt_path"] + "/weights.weights.h5")
            print(f"[ckpt] saved to {params['model_ckpt_path']}")
        if params["method"] == "ES-DFM" or params["method"] == "DFM":
            continue
