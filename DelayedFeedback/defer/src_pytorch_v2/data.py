...
def prepare_data(train_df, test_df, feature_cols, encoder, method='oracle', window=24):
    ...
    if method == 'vanilla':
        ...
        train_labels = {
            ...
            'delay_time': train_df.get('delay_time', np.zeros(len(train_df))).astype(np.float32),
        }
        print(f"  使用标签: label_oracle + delay_time")
    
    elif method == 'esdfm':
        # ES-DFM: 需要 tn/dp/pos 标签
        train_labels = {
            'label': train_df['label_oracle'].values.astype(np.float32),
            'tn_label': (train_df['label_oracle'] == 0).values.astype(np.float32),  # 真负
            'dp_label': np.zeros(len(train_df), dtype=np.float32),  # 延迟正 (需要额外数据)
            'pos_label': train_df['label_24h'].values.astype(np.float32),  # 窗口内正
        }
        print(f"  使用标签: label_oracle + tn/dp/pos labels")
